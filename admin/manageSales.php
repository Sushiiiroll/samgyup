		...
		<div class="container-fluid">
			...
			<div class="card">
				<div class="card-header">
					...
					<button class="btn btn-outline-primary float-right" id="download-pdf"><i class="fa-solid fa-plus"></i> Generate PDF</button>
							...
							<tbody>
								...
								WHERE s.order_number = '$order_no'" . $filter;
										...
										while ($order_row = $res->fetch_assoc()) {
											...
											$count++;
											echo '<tr>';
											echo '<td>' . $order_row['order_number'] . '</td>';
											echo '<td>' . $order_row['menu'] . '</td>';
											echo '<td>' . $order_row['order_category'] . '</td>';
											echo '<td>' . $order_row['quantity'] . '</td>';
											echo '<td>' . $discount . '</td>';
											echo '<td>' . $order_row['order_type'] . '</td>';
											echo '<td>' . $order_row['date'] . '</td>';
											echo '</tr>';
										}
									}
...
<script>
	...
	document.getElementById('download-pdf').addEventListener('click', function() {
		...
		// Set the row data and colors
		var data = [];
		var colors = ["#f5f5f5", "#e8e8e8"]; // Add as many colors as needed
		var colorIndex = 0;
		...
		// Add title
		doc.setFontSize(10);
		doc.text('Zamgy Sales Report', 15, 20);
		var itemsCount = data.length-1;
		doc.text('Total number of records: ' + itemsCount.toString(), 15, 30);
		// Set the font type and size for the headers
		...
		for (var i = 0; i < headers.length; i++) {
			doc.text(x, 40, headers[i]);
			x += 26;
		}

		// Set the font type and size for the row data
		doc.setFont("helvetica", "");
		doc.setFontSize(8);

		// Loop through the row data and add it to the PDF
		var y = 45;
		for (var i = 0; i < data.length; i++) {
			var row = data[i].data;
			var x = 15;

			// Add the background color for the row
			doc.setFillColor(data[i].color);
